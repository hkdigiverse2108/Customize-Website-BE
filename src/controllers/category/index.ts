...
export const getCategories = async (req, res) => {
  ...
  try {
    ...
    const populate = [
      { path: "storeId", select: "_id name" },
      { path: "parentCategoryId", select: "_id name" },
    ];

    const categories = await findAllAndPopulate(categoryModel, criteria, {}, options, populate);
...
export const getCategoryById = async (req, res) => {
  reqInfo(req);
  try {
    const val = validate(categoryIdSchema, req.params, res);
    if (!val) return;

    const comp: any = await getFirstMatch(categoryModel, { _id: val.id, isDeleted: { $ne: true } }, {}, {});
