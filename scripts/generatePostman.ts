...
async function main() {
  ...
  const resolvedBaseUrl = process.env.BACKEND_URL || `http://localhost:${process.env.PORT || 3000}`;
  // @ts-ignore
  collection.variables.add({ key: "base_url", value: resolvedBaseUrl });
  // @ts-ignore
  collection.variables.add({ key: "BASE_URL", value: resolvedBaseUrl });
  // @ts-ignore
  ...
  for (const [routerName, basePath] of Object.entries(basePaths)) {
    ...
    const hasAuthMiddleware = routes.some((r) =>
      r.middleware.some((m) => /(jwt|auth|allowroles)/i.test(m))
    );
